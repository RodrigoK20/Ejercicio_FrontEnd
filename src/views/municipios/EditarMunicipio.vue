<template>
    ...
    <v-container>
        ...
        <v-row>
            <v-col class="mb-4">
                <h1 class="display-2 font-weight-bold mb-3">Editar municipio</h1>
            </v-col>
        </v-row>

        <v-row>
            <v-col>
                <form v-on:submit.prevent="actualizarMunicipio()">
                  <v-text-field
...
let url = 'http://localhost:8000/api/municipio/edit/';
...
export default {
    ...
    //Traer datos del backend al formulario
       created() {
            this.axios
                .get(`http://localhost:8000/api/municipio/edit/${this.$route.params.id}`)
                .then((response) => {
        ...
        methods:{
            ...
            actualizarMunicipio(){
                ...
                this.axios.post(`http://localhost:8000/api/municipio/update/${this.$route.params.id}`, this.municipio).then(response=>{
                    console.log(response);
                         this.$swal({
                             icon: 'success',
